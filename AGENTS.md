...
- Вход: `main.js` (главный процесс Electron, окно 900×1000 фиксированное — `resizable/maximizable false`, безрамочное `frame:false + transparent`, скругление рисует CSS `.app`, single-instance) → `preload.js` открывает только `window.electronAPI.{loadData, saveData, clearData, minimizeWindow, closeWindow}` (`contextIsolation: true`, `sandbox: true`, `nodeIntegration: false`) → `index.html` (есть CSP-meta) → `js/main.js` — точка входа рендера.
- Модули рендера — IIFE-глобалы на `window.*` (`Utils`, `Validate`, `Theme`, `Precision`, `Data`, `Skeleton`, `Modal`, `History`, `Edit`, `InputFilter`, `Export`, `Calculator`, `WindowControls`). Никаких `import`/`require`. **Порядок тегов `<script>` в `index.html` и есть порядок загрузки**: `utils → shared/validate → theme → precision → data → skeleton → modal → history → edit → input-filter → export → calculator → window-controls → main`, а `js/main.js` явно вызывает каждый `init()` — новые модули нужно добавлять и в список тегов, и в init `js/main.js`.
- То же для CSS: `variables.css` обязан быть первым; полный порядок — в `index.html` / README.
- Хранение разделено: история идёт через IPC в `areas_history.json` в Electron `userData` (`main.js` обрабатывает `load-data` / `save-data` / `clear-data` + `window-minimize` / `window-close` для своей панели); тема и точность лежат в `localStorage`. Вне Electron (`window.electronAPI` отсутствует) `Data.load()` ловит ошибку и возвращает `[]`.
